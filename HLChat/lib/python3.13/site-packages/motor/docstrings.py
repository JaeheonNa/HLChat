...
:Parameters:
  ...
  - `bypass_document_validation`: (optional) If ``True``, allows the
    write to opt-output of document level validation. Default is
    ``False``.
  - `session` (optional): a
    :class:`~pymongo.client_session.ClientSession`, created with
    :meth:`~MotorClient.start_session`.
...
:Parameters:
  ...
  - `output`: output collection name or `output object` (dict). See
    the `map reduce command`_ documentation for available options.
    Note: `output` options are order sensitive. :class:`~bson.son.SON`
