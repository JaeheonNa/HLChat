...
class AgnosticDatabase(AgnosticBaseProperties):
    ...
    def aggregate(self, pipeline, *args, **kwargs):
        ...
        :class:`MotorDatabase`, except when ``$output`` or ``$merge`` are used, in
...
class AgnosticCollection(AgnosticBaseProperties):
    ...
    def aggregate(self, pipeline, *args, **kwargs):
        ...
        an aggregation with ``$output`` or ``$merge`` the application needs to
        iterate the cursor, for example::

           cursor = motor_coll.aggregate([{'$output': 'output'}])
           # Iterate the cursor to run the $output (or $merge) operation.
           await cursor.to_list(length=None)
           # Or more succinctly:
           await motor_coll.aggregate([{'$output': 'output'}]).to_list(length=None)
           # Or:
           async for _ in motor_coll.aggregate([{'$output': 'output'}]):
               pass
        ...
        .. versionchanged:: 2.1
           ...
           containing the `$output` stage when connected to MongoDB >= 4.2.
