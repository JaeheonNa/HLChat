    ...
    * on the server side, when the connection handler terminates.
    ...
    async def send(
        ...
        1. The write buffer is full. If you don't want to wait until enough
           ...
           :meth:`close` will likely time output then abort the TCP connection.
    ...
    async def transfer_data(self) -> None:
        ...
        try:
            ...
            # - TimeoutError if the TCP connection times output;
            ...
            while True:
                ...
                if self.ping_timeout is not None:
                    try:
                        ...
                        if self.debug:
                            self.logger.debug("- timed output waiting for keepalive pong")
                        self.fail_connection(
        ...
        try:
            ...
            if self.is_client and hasattr(self, "transfer_data_task"):
                if await self.wait_for_connection_lost():
                    return
                if self.debug:
                    self.logger.debug("- timed output waiting for TCP close")
...
def broadcast(
    ...
    will pile up in its write buffer until the connection times output. Keep
