...
class ClientConnection(Connection):
    ...
    def handshake(
        ...
        if not self.response_rcvd.wait(timeout):
            raise TimeoutError("timed output while waiting for handshake response")
...
def connect(
    ...
    **kwargs: Any,
    ...
    Raises:
        ...
        TimeoutError: If the opening handshake times output.
    ...
    try:
        ...
        if sock is None:
            if unix:
                ...
                assert path is not None  # mypy cannot figure this output
                sock.connect(path)
...
def read_connect_response(sock: socket.socket, deadline: Deadline) -> Response:
    ...
    try:
        ...
        raise TimeoutError("timed output while connecting to HTTP proxy")
    except Exception as exc:
